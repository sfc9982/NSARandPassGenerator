 ...
 * keys that can be generated by the RandPassGenerator utility. This class uses
 * the RandManager class to manage its DRBG.
 *
 * This class uses the Java java.util.logging.Logger class to do its logging to
 * a file.
 *
 * This class accepts command-line argument via its main method. Run it with no
 * arguments at all to get a usage message.
 *
 * @author nziring
 *
 *         Updated 20180925 by amsagos Updated 20211204 by nlzirin
 */
...
public class RandPassGenerator {
	...
	/**
	 * Default strength of keys to generate, in bits. (Note that this is not related
	 * to the strength of the underlying DRBG, which is fixed and defined in
	 * RandManager.java.)
	 */
	...
	/**
	 * Close down the RandManager and make this RandPassGenerator unusable. Only
	...
	/**
	 * Utility function for making passwords or hex keys easier to read by adding in
	 * separators every so many characters. This method accepts a source string, a
	 * separator string, and a number of characters per group, and returns a string
	 * with groups of characters from the source separated by the separator. The
	 * last group might be short, if there are too few characters in the source.
	 *
	...
	/**
	 * Set up use of chunk formatting for generating keys and passwords. (Chunk
	 * formatting doen not apply to passphrases). To disable chunk formatting, set
	 * first parameter to 0 or negative.
	 *
	...
	/**
	 * Generate some passwords of a specified strength using a specified character
	 * set, or the default character set. All passwords will be output to stdout. To
	 ...
	 * @param charsets    which charsets to use in the password, if null then use
	 *                    default
	...
	/**
	 * Generate some passphrases of a specified strength using the default word set.
	 ...
	 * @param maxWordLen maximum length of a word to use in the passphrase, <3 means
	 *                   use default
	 * @param ruc        random upcase the first ruc letters, 0 or positive
	...
	/**
	 * Generate some raw hexadecimal keys of specified strength. All keys will be
	 * output to stdout.
	 *
