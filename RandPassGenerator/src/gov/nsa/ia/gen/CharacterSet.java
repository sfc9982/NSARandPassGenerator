...
/**
 * The CharacterSet class represents a character list from which a password
 * could be created. Callers can then ask for a string of any length, composed
 * of characters chosen uniformly from the set. Multiple instances of this class
 * could potentiall be used for creating more complexified passwords (e.g.,
 * 'pronouncable' passwords, passwords that obey silly complexity rules, or
 * passwords that have to pass character class presence checks) but by itself
 ...
 * characters, which would change the relative frequency of characters in the
 * output. However, the set is de-duped so that both sampling and entropy
 * calculations use only unique characters in the character set. Therefore, it
 * is not possible to use this class to make a password where some characters
 * (e.g., lowercase letters) appear more often than others (e.g., digits). Such
 * passwords might be useful in some contexts, but this class will not make
 * them.
 *
 * @author Neal Ziring Updated by amsagos
 */
...
public class CharacterSet {
	...
	/**
	 * Create an empty CharacterSet object. The addSet method must be called at
	 * least once before this CharacterSet is usable.
	 *
	...
	/**
	 * Create a CharacterSet object, and addSet the given String. All the chars in
	 * the String will be added to the charset using addSet.
	 *
	 * @param lg initialized Logger for logging
	...
	/**
	 * Add a bunch of characters to the CharacterSet. It is the responsibility of
	 * the caller to look out for duplicate characters; if they're passed to this
	 * method, they'll be kept and the probability of that character being in the
	...
	/**
	 * Given an array of chars, return an array of chars of equal or shorter length,
	 * containing only one of each char that appears in the original. This algorithm
	 * runs in time O(n * m) for n the number of chars in the input and m the number
	 * of unique chars.
	 *
	 ...
	 * characters in the set, rather than just size of the set. Later changed this
	 * class so that it ALWAYS uses the de-duped character set, eliminating the need
	...
	/**
	 * Return the next uniformly distributed short int on the range [0, maxval).
	 * This uses a buffer of shorts which we get from the drbg, refilling the buffer
	 * when needed. If anything goes wrong, we return null.
	 *
	 * Note that this method uses a simple resampling method to ensure that the
	 * values we return are really uniformly distributed over the interval [0,max).
	 *
	...
	/**
	 * Fill a buffer with positive short integers, so that the caller can have lots
	...
	/**
	 * Generate a string of the given length using randomness taken from the
	 * supplied DRBG.
	 *
	...
	/**
	 * Generate a string with the requested amount of entropy from the given DRBG.
	 * This works by calculating a length and calling getRandomString. If the
	 * strength requested is greater than the overall strength of the DRBG, then we
	 * return null. On any error, we return null.
	 *
