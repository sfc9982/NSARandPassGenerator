...
/**
 * An instance of WordSet class loads up a dictionary, and then generates random
 * passphrases composed of works from the dictionary. Callers can specify a
 * length range for the words, which effectively means that the passphrases can
 * be generated from a subset of the dictionary. Note that, when loading up the
 * dictionary, duplicate words are dropped, so that the final word set contains
 * one of each unique word from the dictionary. This means that all words in the
 * dictionary or length-constrained subset of the dictionary, have equal chance
 * of appearing in the output passphrase.
 *
 * The mechanisms used in this class are not especially efficient, but they're
 * not too bad - for the most frequently used operation, getRandomWord, they're
 * only about 4X worse than minimum possible.
 *
...
public class WordSet {
	...
	/**
	 * Create the WordSet, using the supplied URL. If null, then attempt to use the
	 * default path.
	 *
	...
	/**
	 * Load up the base wordset from the given URL. Returns the count of words
	 * loaded, or -1 on error. All activities logged.
	 *
	 * POST-REVIEW NOTE: During loading, a HashSet is used, so that duplicate words
	 * are ignored. The final set used will be the unique words that appeared in the
	 * URL.
	 *
	...
	public int loadBaseWordsFromURL(URL src) {
		...
		try {
			...
			logger.info("WordSet - loaded " + cnt + " words from data file, accepted " + wordset.size()
					+ " unique words into candidate set.");
			try {
				br.close();
			} catch (Exception e2) {
	...
	/**
	 * Set the length range for returning words. When words are later retrieved from
	 * this WordSet, it won't return words of length outside the range. (This works
	...
	public int setLengthRange(int minlen, int maxlen) {
		...
		if (maxlen < 2)
			throw new IllegalArgumentException("Maximum length must be > 1");
		if (minlen >= maxlen)
	...
	/**
	 * Return the number of bits of entropy that can be expected in one word
	 * selected from the set. Basically log2(size).
	 */
	...
	/**
	 * Retrieve a random word from the wordset, given a properly instantiated
	 * AbstractDRBG. If anything goes wrong, we return null.
	 *
	 * This implementation uses a very simple approach to getting a uniformly
	 * distributed choice over the length-selected subset of words.
	 * <ol>
	 ...
	 * </ol>
	 *
	 * This is a rather wasteful of DRBG output bits, in the sense that on average
	 * it will call getInteger() N times for 32 bits of random each time
	 * (N=32/log2(M)) when it really needs only log2(M) bits. (M usually about 64K,
	 * so would be 16-17 bits, so we'll typically need to call generateInteger about
	 * twice, therefore consuming about 64 bits of random - 4x as much as we really
	 * need to.) There is an optimal method I could have used, called the Han-Hoshi
	 * algorithm, but it is considerably more complex and difficult to ensure
	 * perfectly uniform selection when M is not a power of 2. So, I went with this
	 * simple method.
	 *
	...
	/**
	 * Get a random list of words of length sufficient to provide the requested
	...
	/**
	 * Randomly upcase the first N letters of a provided word, drawing random from
	 * the given AbstractDRBG. If N is greater than or equal to the length of the
	 * input string, then all the letters are potentially subject to uppercase.
	 *
	...
	static byte[] nonceBytes = { (byte) 0x01, (byte) 0x02, (byte) 0x01, (byte) 0x02, (byte) 0x01, (byte) 0x02,
	...
	/**
	 * main for unit testing - this main creates a HashDRBG and then generates a
	 * large number of test words.
	 *
	 * The first command-line arg should be a URL of a word file. On most Linux
	 * systems, file:///usr/share/dict/words will work.
	 */
