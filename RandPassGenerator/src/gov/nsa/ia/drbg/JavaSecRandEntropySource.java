...
/**
 * This class supports the Get_entropy_input function of SP800-90. This
 * implementation uses multiple instances of the Java java.security.SecureRandom
 * generator, which uses entropy gathered from an OS-specific mechanism or
 * mechanisms. Note that we use the generateSeed() method, which grabs raw
 * system entroy, so it may block.
 *
 * This implementation also supports self-test.
 *
 * This entropy source is suitable for use on most operating systems where Java
 * runs, including Linux and Windows. But note that it sucks up system entropy
 * fairly aggressively, so don't be surprised if it blocks.
 *
 * @author nziring
 */
...
public class JavaSecRandEntropySource implements EntropySource, SelfTestable {
	...
	/**
	 * Default number of bytes to generate at a time, evenly split across the number
	 * of SecureRandom instances. Must be an even mulitple of DEFAULT_SOURCES.
	 */
	public static final int BLOCK_SIZE = 16;

	/**
	 * Expected bits of entropy available from the SecureRandom sources per
	 * BLOCK_SIZE bytes of entropy obtained from the SecureRandom generateSeed. We
	 * assume that all the bits are good, because the HashDRBG already asks for lots
	 * of extra, so we assume full goodness here.
	 */
	...
	/**
	 * Create this EntropySource object, including initializing the underlying
	 * SecureRandom source objects.
	 *
	 * May throw a GeneralSecurityException if the underlying crypto subsystem is
	 * unable to instantiate a SecureRandom object.
	 */
	...
	public String toString() {
		...
		int i;
		for (i = 0; i < DEFAULT_SOURCES; i++) {
			sb.append(", SecureRandom " + (i + 1) + " provider=" + sources[i].getProvider().toString());
		}
		sb.append("]");
	...
	/**
	 * Get requested entropy. This method returns the entropy as a byte array. It
	 * gets the entropy by reading from multiple Java SecureRandom instances. Note
	 * that this call may block until entropy is available, because it uses the
	 * SecureRandom generateSeed() method. If the entropy cannot be delivered at
	 * all, then STATUS_ERROR is returned. If the bit amount of entropy requested
	...
	public byte[] getEntropy(int requestedEntropyBits, int minOutputBytes, int maxOutputBytes) {
		...
		blocks = (int) Math.ceil((double) requestedEntropyBits / (double) BITS_ENTROPY_PER_BLOCK);
	...
	/**
	 * Fill a block buffer by calling each SecureRandom in turn. Return false if
	 * anything goes wrong, true on success.
	 *
	...
	/**
	 * Perform a basic self-test on this entropy source. This entails running
	 * calling the getEntropy method several times and checking that the correct
	 * behavior occurs. For those calls which generate entropy, we do a simple
	 * byte-entropy computation and make sure it exceeds a threshold.
	 *
	...
	/**
	 * Return entropy gathered during self-test (case 2). The block returned will be
	 * of size 2*BLOCK_SIZE, and will have passed a basic check performed by
	 ...
	 * performSelfTest(). Then it calls getEntropy and prints the output. If there
	 * is a command-line argument, then additional self-test will be performed.
	 */
