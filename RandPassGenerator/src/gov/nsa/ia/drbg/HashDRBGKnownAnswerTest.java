...
import gov.nsa.ia.util.FixedValuesEntropySource;

/**
 * A simple little class to hold the values associated with a HashDRBG
 * known-answer test. The steps in a known-answer test are always the same in
 * the Hash_DRBG.txt test vectors file supplied by NIST.
 *
 * Step 1 - Create the DRBG
 *
 * Step 2 - Instantiate the DRBG with particular value for
 * strength/entropy-input-len and fixed values for the entropy input, nonce, and
 ...
 *
 * Each instance of this class instantiates a FixedValuesEntropySource to supply
 * the fixed entropy input values.
 */
class HashDRBGKnownAnswerTest {
	...
	/**
	 * Set values for step 3: entropyInput, postV, and PostC. Step 3 value for add'l
	 ...
	 * expected output. Note that add'l input is always null.
	 */
	...
	/**
	 * Return the pre-loaded entropy source. This has to be called after setStep3 or
	 * later.
	 */
