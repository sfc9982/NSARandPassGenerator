...
public class EntropyUtil {
	...
	/**
	 * Compute the chi-squared statistic of a byte buffer, using bit chunks of the
	 ...
	 * @return pearson's chi-squared statistic for the samples, based on hypothesis
	 *         of uniform distro
	 */
	public static double chiSquaredStatistic(byte[] buf, int len, int chunkBits) {
		...
		if (chunkBits < 2 || chunkBits > MAX_CHISQ_SAMPLE_BITS)
			throw new IllegalArgumentException("Chunk size invalid, must be >=2 and <=" + MAX_CHISQ_SAMPLE_BITS);
	...
	public static boolean testChiSquared(double chisq, int degfr) {
		if (chisq < 0.0)
			throw new IllegalArgumentException("Bad chi-squared statistic value");
		if (degfr < 2 || degfr > MAX_CHISQ_SAMPLE_BITS)
			throw new IllegalArgumentException("Invalid sample size, must be >1 and <=" + MAX_CHISQ_SAMPLE_BITS);
	...
	/**
	 * String to byte buf utility, for accepting buffers more easily. This
	 * implementation adapted from the Sun source code from
