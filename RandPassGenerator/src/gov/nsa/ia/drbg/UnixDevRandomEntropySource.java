 ...
 * implementation uses the Unix/Linux /dev/random pseudo-file, which delivers
 * quality entropy gathered from internal kernel operations. This implementation
 * also supports self-test and returning entropy gathered during self-test.
 *
...
public class UnixDevRandomEntropySource implements EntropySource, SelfTestable {
	 ...
	 * entropy for them or not.
	 */
	public static final String DEFAULT_RANDOM_SOURCE = "/dev/random";

	/**
	 * Default buffer size to read from the RANDOM_SOURCE, we always read this much
	 * at a time. (this is in bytes)
	 */
	public static final int BLOCK_SIZE = 32;

	/**
	 * Expected bits of entropy available from RANDOM_SOURCE per 32 bytes of data
	 * obtained. Basically, we're expecting .5 bits of entropy from every bit of
	 * output from system /dev/random. Since the system /dev/random on most Unix
	 * systems are designed to deliver fully conditioned randomness and block until
	 * it can do so, 0.75 bits is a conservative bound.
	 */
	...
	/**
	 * Random source that this instance uses, normally DEFAULT_RANDOM_SOURCE.
	 */
	...
	/**
	 * Create this EntropySource object, including opening the random source. This
	 * constructor will throw an IOException if the underlying random source cannot
	...
	/**
	 * Create this EntropySource object, using a specified random source instead of
	 ...
	 * gets the entropy by reading from Linux or Unix /dev/random. may block until
	 * entropy is available. If the entropy cannot be delivered at all, then
	 * STATUS_ERROR is returned. If the bit amount of entropy requested cannot be
	...
	/**
	 * If set to true, performSelfTest will do an extra round of testing. This
	 * should not be used during normal operation because it will suck too much
	 * entropy out of /dev/random. This can be used to impose additional
	 * self-testing when examining a new platform.
	 */
