...
/**
 * This class support the Get_entropy_input function of SP800-90.
 * Implementations of this interface must deliver entropy upon request, though a
 * request may block until enough entropy is available. This particular class
 * implements reading some entropy from a file, and also writing a bunch of
 * entropy to a file. It is used to save some entropy from a running DRBG for
 * use next time, or for reading data with modest entropy from the /proc
 * filesystem files.
 */
public class FileEntropySource implements EntropySource, SelfTestable {
	...
	/**
	 * default block size in bytes to read from the source.
	 */
	protected int blockSize;

	/**
	 * Expected bits of entropy available from RANDOM_SOURCE per block of bytes
	 * obtained.
	 */
	protected int entropyBitsPerBlock;

	/**
	 * File into which to save entropy, if different from file from which we read
	 * entropy.
	 */
	...
	/**
	 * Set the destination File for saving entropy. If set to null, then saving
	 * entropy is explicitly disabled.
	 */
	 ...
	 * gets the entropy by reading from the give file. It may block until entropy is
	 * available. Note that each time this is called we read the file anew. This
	 * works well for pseudo-files that deliver something a little different each
	 * time, but is lousy for normal files. If the file cannot be read, then null is
	...
	/**
	 * Write entropy to the defined destination, reading that entropy from an
	 * already-instantiated DRBG. (Note that the destination is, by default, the
	 * same as the source. To set a different destination, use setDestination.)
	 *
	...
	/**
	 * Return the failure message from the performSelfTest method. If the method
	 * hasn't been called, or the test succeeded, then this method returns null.
	 */
	 ...
	 * byte-entropy computation and make sure it exceeds a threshold. This self-test
	 * is fairly complex so you can call getSelfTestFailureMsg() to obtain a value
	 * describing why the self-test failed.
	 *
	...
	/**
	 * Simple unit test main method, this main method expects one or more files to
